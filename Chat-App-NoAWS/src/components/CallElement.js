...
const CallElement = ({ img, name, id, handleClose }) => {
  ...
  const handleAudioCall = () => {
    // Dispatch an action to start an audio call
    dispatch(StartAudioCall(id));
    handleClose();
  };

  useEffect(() => {
    const handleAudioCallResponse = (data) => {
      // Handle the response based on your application logic
      console.log('Received audio call response:', data);
      // You can implement UI changes based on the response
    };

    // Listen for audio call responses
    socket.on('audio_call_response', handleAudioCallResponse);

    // Clean up the event listener when component is unmounted
    return () => {
      socket.off('audio_call_response', handleAudioCallResponse);
    };
  }, []);
