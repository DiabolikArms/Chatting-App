...
io.on("connection", async (socket) => {
  ...
  socket.on("start_audio_call", async (data) => {
    ...
    // Create a record of the audio call
    const audioCallRecord = await AudioCall.create({
      participants: [to, from],
      roomID: roomID,
      status: "Initiated",
      startedAt: Date.now(),
    ...
    // send notification to receiver of the call
    io.to(to_user?.socket_id).emit("audio_call_notification", {
      ...
      userID: to,
      userName: to,
      callRecordID: audioCallRecord._id, // Send the call record ID to the receiver
    });
  });
  ...
  socket.on("audio_call_not_picked", async (data) => {
    ...
    // Find and update the audio call record to mark it as missed
    ...
    const to_user = await User.findById(to);
    // Emit call_missed to the caller (from user)
  ...
  socket.on("audio_call_accepted", async (data) => {
    ...
    // Find and update the audio call record to mark it as accepted
    await AudioCall.findOneAndUpdate(
      {
        participants: { $size: 2, $all: [to, from] },
      },
    ...
    const from_user = await User.findById(from);
    // Emit call_accepted to the sender of the call
  ...
  socket.on("start_video_call", async (data) => {
    ...
    // Create a record of the video call
    const videoCallRecord = await VideoCall.create({
      participants: [to, from],
      roomID: roomID,
      status: "Initiated",
      startedAt: Date.now(),
    ...
    io.to(to_user?.socket_id).emit("video_call_notification", {
      ...
      userID: to,
      userName: to,
      callRecordID: videoCallRecord._id, // Send the call record ID to the receiver
    });
  });
  ...
  socket.on("video_call_not_picked", async (data) => {
    ...
    // Find and update the video call record to mark it as missed
  ...
  socket.on("video_call_accepted", async (data) => {
    ...
    // Find and update the video call record to mark it as accepted
    await VideoCall.findOneAndUpdate(
      {
        participants: { $size: 2, $all: [to, from] },
      },
  ...
  // Handle 'call:initiate' event
  socket.on('call:initiate', ({ callId, participants }) => {
    // Emit 'call:incoming' event to each participant
    participants.forEach((participant) => {
      io.to(participant).emit('call:incoming', { callId, initiator: socket.id });
    });
  });

  // Handle 'call:accept' event
  socket.on('call:accept', ({ callId, initiator }) => {
    // Emit 'call:accepted' event to the initiator
    io.to(initiator).emit('call:accepted', { callId, acceptor: socket.id });
  });

  // Handle 'call:reject' event
  socket.on('call:reject', ({ callId, initiator }) => {
    // Emit 'call:rejected' event to the initiator
    io.to(initiator).emit('call:rejected', { callId });
  });

  // Handle 'call:end' event
  socket.on('call:end', ({ callId, participants }) => {
    // Emit 'call:ended' event to each participant
    participants.forEach((participant) => {
      io.to(participant).emit('call:ended', { callId });
    });
  });
});
